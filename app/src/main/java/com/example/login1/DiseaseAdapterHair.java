...
public class DiseaseAdapterHair extends RecyclerView.Adapter<DiseaseAdapterHair.ViewHolder> {

    private Context context;
    private List<DiseaseAdmin>diseaseList;

    public DiseaseAdapterHair(Context context, List<DiseaseAdmin> diseaseList) {
    ...
    @Override
    public void onBindViewHolder(@NonNull DiseaseAdapterHair.ViewHolder holder,int position) {
        DiseaseAdmin disease = diseaseList.get(position);
        holder.name.setText(disease.getName());
        holder.itemView.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                Intent intent = new Intent(context, display_hair_admin.class);
