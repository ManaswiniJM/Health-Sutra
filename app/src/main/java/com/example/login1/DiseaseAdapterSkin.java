...
public class DiseaseAdapterSkin extends RecyclerView.Adapter<DiseaseAdapterSkin.ViewHolder> {

    private Context context;
    private List<DiseaseAdmin>diseaseList;

    public DiseaseAdapterSkin(Context context, List<DiseaseAdmin> diseaseList) {
    ...
    @Override
    public void onBindViewHolder(@NonNull DiseaseAdapterSkin.ViewHolder holder, int position) {
        DiseaseAdmin disease = diseaseList.get(position);
        holder.name.setText(disease.getName());
        holder.itemView.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                Intent intent = new Intent(context, display_skin_admin.class);
